...
| 网络 | 小 **ResNet**；输入 **`15×10×9`**；训练默认可含 **policy + attack / danger / tactical** 等辅助头；导出 ONNX 见下文契约（可选仅 **`logits`**）——设计上避免把网络当作 **完整 Pikafish 式静态评估的替代品** |
...
- **输出**：
  ...
  - **多头（`aux_heads`，默认训练开启）**：在上述基础上追加 **`attack`、`danger`、`tactical`**，均为 **`float32[1]`**。**ONNX 图中**三辅助输出已含 **sigmoid**（见 `export_onnx.py` 的 `PolicyOnnxExport`），数值约在 `[0,1]`；训练仍对原始 logit 做 sigmoid 后与伪标签对齐。语义头设计目的是 **人类式局面提示** 与 **搜索调度**，不是单独复刻引擎评估曲线。
  - **可选 `value` 输出**（若训练导出）：见 **`TODO.md` P4**；语义上为 **人类局面感 / 剪枝辅助**，与 **`engin`** 叶子评估（可与物质 fallback 协作）一致即可。
