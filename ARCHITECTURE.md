...
- **短期主产品：复盘系统**
  ...
  - 当前最小复盘系统目标是 **`policy + value + danger + attack`**
  - 直接服务候选着解释、局势趋势、危险提示、攻势判断、复盘分析
  - 复盘系统可结合**真理引擎**与**滑动窗口前后文**
- **长期路线：搜索引擎**
...
| 网络 | 小 **ResNet**；输入 **`15×10×9`**；当前训练主线是**先做强共享 trunk / policy，再冻结 trunk 分别读出 `value / danger / attack`**；`tactical` 暂不属于复盘 MVP；导出 ONNX 见下文契约（可选仅 **`logits`**）——设计上避免把网络当作 **完整 Pikafish 式静态评估的替代品** |
...
- **复盘系统允许结合外部真理引擎与前后手滑动窗口上下文**：模型负责候选着与局面语义，真理引擎负责战术校验与客观对错，窗口负责把单点判断扩展成阶段性解释。
...
- **语义**：先让网络学会「人类关心的局面语义」（趋势、危险、进攻压力等），再谈盲目扩数据量。
- **多头**：当前复盘 MVP 以 **policy + value + danger + attack** 为主；`tactical` 进入第二波。标签宜 **规则/浅规则自动生成**，服务于 **复盘解释** 与后续 **搜索调度**，而非替代引擎全文搜索。
