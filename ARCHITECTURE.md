...
`crates/engin`：px0 的 UCI/controller、网络外围与 MCTS Rust 翻译；不在搜索内复制规则。当前 P2 UCI、P3 单 worker 树与 P4 的 ONNX/worker 子集已存在；完整 collision/task-worker/prefetch 仍待逐函数翻译。`WeightsFile` 保持 px0 的 UCI 名称，但只接受本项目 ONNX 模型，不实现 px0 的 backend registry 或 protobuf weight discovery。
