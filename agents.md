...
- 已明确偏离 px0/Lc0 stream 基线：无 multivisit、prefetch、tree-batch gather。PUCT 使用 edge in-flight reservation 作为 virtual visit；每个 Gather event 是一份 visit，碰撞取消未完成路径的 reservation。NN 只消费连续的已编码 tensor 队列，不等待逻辑搜索轮次。
