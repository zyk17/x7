...
impl NodeArena {
    ...
    pub fn spawn_child(&mut self, parent_idx: usize, edge_idx: usize) -> usize {
        if let Some(child_idx) = self.get(parent_idx).expect("valid parent node").child(edge_idx) {
            return child_idx;
        }
...
impl NodeTree {
    ...
    pub fn make_not_terminal(&mut self, node_idx: usize) {
        let child_indices: Vec<usize> = self.node(node_idx).children.iter().filter_map(|child| *child).collect();
    ...
    pub fn make_solid(&mut self, node_idx: usize) -> bool {
        ...
        for child_idx in node.children.iter().filter_map(|child| *child) {
...
mod tests {
    ...
    /// px0 `GetOrSpawnNode` returns the existing child or creates exactly one
    /// while the caller owns the tree phase (`src/search/classic/node.h:468-525`,
    /// `search.cc:1142-1211,1494-1508`).
    #[test]
    fn spawn_child_reuses_existing_child_within_tree_phase() {
        let mut arena = NodeArena::default();
        let root = arena.alloc(Node::default());
        arena.get_mut(root).expect("root").create_single_child_node(Move::new(
            xiangqi_core::Square::parse("a0").expect("a0"),
            xiangqi_core::Square::parse("a1").expect("a1"),
        ));

        let first = arena.spawn_child(root, 0);
        assert_eq!(arena.spawn_child(root, 0), first);
        assert_eq!(arena.get(root).expect("root").child(0), Some(first));
    }
